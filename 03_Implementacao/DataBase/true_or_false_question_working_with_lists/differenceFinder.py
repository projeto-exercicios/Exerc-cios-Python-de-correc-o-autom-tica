...
def questionHTML(fileName, pdfVersion = False, prefix = 'question/version_1/'):
    ...
    file2 = open(fileName,'r',encoding='utf_8') if prefix == '' else open('simulated_tex.tex','r',encoding='utf_8')
    fileSplitted2 = re.split('\n|\n\n',file2.read())
    ...
    for idx in rangeValue:
        ...
        if pdfVersion and idx in  idxCode:
            ...
            continue
        text1 = format_string(fileSplitted[idx].split(" "))
        text2 = format_string(fileSplitted2[idx].split(" "))
...
def simulateTex(text, prefix):
    ...
    if text in find_general_files_to_change():
        ...
        print(text_module.to_string())
##        print(seed_value)
##        python_module1 = load_source_code_from_file('altered_full_program.py')
##        python_module1.execute()
##        print(python_module1.to_string())
##        foo.make_transformations_on_results(python_module1)
##    create_file('simulated_tex.tex', text_module.to_string())
    simulateTex2(text_module.to_string(),prefix, seed_value)

def simulateTex2(text, prefix, seed_value):
    '''
        Esta função com a ajuda do "alter_make_transformations" consegue repro
        duzir o código de python com as tags de html necessária para identificar
        onde occoreu uma alteração em comparação com o ficheiro de python
